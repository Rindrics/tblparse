 ...
 */
export function detectTableBlocks(sheet: XLSX.WorkSheet, labelColumn: string = "A"): TableBlock[] {
...
export function detectHeaderRow(
  ...
  if (headerPattern) {
    const match = dataRows.find((r) => r.labelValue && headerPattern.test(r.labelValue));
    if (match) {
      return match;
    }
  }
